...
// Фото профиля — пути, которые клиент потом подставляет в <img src>. Писать в БД
// что угодно нельзя: туда можно положить javascript:, data: или внешний адрес для
// отслеживания. Разрешаем только собственные загрузки (/uploads/...) и https.
const MAX_PHOTOS = 12;
...
function sanitizePhotos(input) {
  ...
  for (const item of input.slice(0, MAX_PHOTOS)) {
    ...
    if (UPLOAD_PATH_RE.test(item) || HTTPS_URL_RE.test(item)) clean.push(item);
  }
  return clean;
}
  ...
  try {
    ...
    if (photos !== undefined) {
      ...
      if (cleanPhotos === null) {
        return res
          .status(400)
          .json({ error: "photos должен быть массивом строк" });
      }
      updates.push("photos = ?");
      values.push(JSON.stringify(cleanPhotos));
    }
