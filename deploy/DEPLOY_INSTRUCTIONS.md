...
## Если после деплоя кто-то видит пустой экран

Виноват старый service worker на устройстве пользователя: до 15.09.2026 он кэшировал
`index.html` «сначала кэш» и отдавал версию прошлой сборки, ссылающуюся на уже
удалённые JS-файлы. Новый `sw.js` при активации сам удаляет старые кэши и забирает
страницы под контроль (`clients.claim()`), но браузер обязан сначала загрузить
изменённый `/sw.js` — обычно это происходит при следующем открытии сайта.

Пользователю достаточно один раз перезагрузить страницу (в браузере — жёсткая
перезагрузка). Проверить, что сервер отдаёт правильный worker:

```bash
curl -s https://platformlove.com/sw.js | grep -o 'platform-love-v[0-9]*'
```

Если имя кэша совпадает с текущим в `public/sw.js` — на сервере свежая версия.
